...
export async function POST(req: Request) {
  try {
    // Extract phone number from form data if available
    // Note: We need to clone the request to read formData twice (once for phone, once for file)
    ...
    await prisma.driver.update({
      where: { id: driverId },
      data: { aadharCardFile: uploaded.ufsUrl, aadharCardFileKey: uploaded.key },
    });
